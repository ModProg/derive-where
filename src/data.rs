...
pub struct Data<'a> {
    ...
    pub skip_inner: Skip,
    /// [`struct@Ident`] of this struct, union or variant, used for implementing [`Debug`](core::fmt::Debug).
    pub ident: &'a Ident,
    /// [`Path`] of this struct, union or variant, used to construct new instances of that item, for example when implementing [`Clone`].
    pub path: Path,
...
impl<'a> Data<'a> {
    ...
    /// Returns `true` if this variant is marked as the [`struct@Default`]. If not a variant, always returns `true`.
