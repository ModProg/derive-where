...
impl Error {
    ...
    pub fn item(span: Span) -> syn::Error {
        syn::Error::new(
            span,
			"derive-where doesn't support items without generics, `skip` attributes or `enum`s implementing `Default`, as this can already be handled by standard `#[derive(..)]`",
		)
    }
    ...
    /// Invalid delimiter in `derive_where` attribute for [`Trait`](crate::Trait)s.
    ...
    /// Missing `default` option on a variant when [`Default`] is implemented for an enum.
